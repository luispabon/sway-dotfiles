...
case "$1" in
	...
	region-to-file)
		...
		echo -n ${SCREENSHOT_FILENAME} | ${COPY_COMMAND}
		;;

	region-to-clipboard)
		grim -t ${CAPTURE_FORMAT} -g "$(slurp -b '#AFAFAFAF' -c '#FF3F3FAF' -s '#00000000' -w 3 -d)" - | wl-copy
	...
	window-to-clipboard)
		grim -t ${CAPTURE_FORMAT} -g "$(swaymsg --type get_tree --raw | jq --raw-output "${filter}")" - | wl-copy
        notify-send.sh "Active window screenshot copied to clipboard"
		echo -n ${SCREENSHOT_FILENAME} | ${COPY_COMMAND}
		;;

	active-display-to-clipboard)
		grim -t ${CAPTURE_FORMAT} -o "$(swaymsg --type get_outputs --raw | jq --raw-output '.[] | select(.focused) | .name')" - | wl-copy
